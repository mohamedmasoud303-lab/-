...
export const exportPropertyStatementToPdf = (financials: any, settings: Settings) => {
    const doc = getArabicDoc('كشف حساب عقار', `عقار: ${s(financials.property?.name)}`, settings);
    const y = 40;
    doc.setFontSize(11);
    doc.text(`إجمالي الإيرادات: ${formatCurrency(financials.totalRentalIncome, settings.currency)}`, 195, y, { align: 'right' });
    doc.text(`إجمالي المصروفات: ${formatCurrency(financials.propertyExpenses + financials.maintenanceCosts + financials.utilityCosts, settings.currency)}`, 195, y + 10, { align: 'right' });
    doc.text(`المتأخرات الحالية: ${formatCurrency(financials.currentArrears, settings.currency)}`, 195, y + 20, { align: 'right' });
    doc.text(`صافي الربح: ${formatCurrency(financials.netResult, settings.currency)}`, 195, y + 30, { align: 'right' });

    const head = [['القيمة', 'البيان']];
    const body = [
        [formatCurrency(financials.totalRentalIncome, settings.currency), 'إجمالي إيرادات الإيجار'],
        [formatCurrency(financials.propertyExpenses, settings.currency), 'مصاريف تشغيلية'],
        [formatCurrency(financials.maintenanceCosts, settings.currency), 'تكاليف الصيانة'],
        [formatCurrency(financials.utilityCosts, settings.currency), 'خدمات ومرافق'],
        [formatCurrency(financials.currentArrears, settings.currency), 'المتأخرات'],
    ];
    (doc as any).autoTable({ head, body, startY: y + 40, styles: { font: 'Cairo', halign: 'right' } });
    doc.save(`Property_Statement_${financials.property?.name}.pdf`);
};

export const exportTenantStatementToPdf = (data: any, settings: Settings) => {
    const doc = getArabicDoc(`كشف حساب مستأجر: ${s(data.tenantName)}`, `تاريخ التقرير: ${formatDate(new Date().toISOString())}`, settings);
    const head = [['الرصيد', 'دائن', 'مدين', 'البيان', 'التاريخ']];
    const body = data.transactions.map((t: any) => [
        formatCurrency(t.balance, settings.currency),
        t.type === 'CREDIT' ? formatCurrency(t.amount, settings.currency) : '-',
        t.type === 'DEBIT' ? formatCurrency(t.amount, settings.currency) : '-',
        s(t.description),
        formatDate(t.date)
    ]);
    (doc as any).autoTable({ head, body, startY: 35, styles: { font: 'Cairo', halign: 'right' } });
    doc.save(`Tenant_Statement_${data.tenantName}.pdf`);
};

export const exportIncomeStatementToPdf = (data: any, settings: Settings, range: string) => {
    const doc = getArabicDoc('قائمة الدخل', range, settings);
    const head = [['المبلغ', 'البند']];
    const body = [
        ...data.revenues.map((r: any) => [formatCurrency(r.amount, settings.currency), r.name]),
        [{ content: formatCurrency(data.totalRevenue, settings.currency), styles: { fontStyle: 'bold' } }, { content: 'إجمالي الإيرادات', styles: { fontStyle: 'bold' } }],
        ...data.expenses.map((e: any) => [formatCurrency(e.amount, settings.currency), e.name]),
        [{ content: formatCurrency(data.totalExpense, settings.currency), styles: { fontStyle: 'bold' } }, { content: 'إجمالي المصروفات', styles: { fontStyle: 'bold' } }],
        [{ content: formatCurrency(data.netIncome, settings.currency), styles: { fontStyle: 'bold', fillColor: [240, 240, 240] } }, { content: 'صافي الدخل', styles: { fontStyle: 'bold', fillColor: [240, 240, 240] } }]
    ];
    (doc as any).autoTable({ head, body, startY: 35, styles: { font: 'Cairo', halign: 'right' } });
    doc.save(`Income_Statement_${range}.pdf`);
};

export const exportTrialBalanceToPdf = (data: any, settings: Settings, date: string) => {
    const doc = getArabicDoc('ميزان المراجعة', `بتاريخ: ${formatDate(date)}`, settings);
    const head = [['رصيد دائن', 'رصيد مدين', 'اسم الحساب', 'رقم الحساب']];
    const body = data.map((a: any) => [
        a.balance < 0 ? formatCurrency(Math.abs(a.balance), settings.currency) : '-',
        a.balance > 0 ? formatCurrency(a.balance, settings.currency) : '-',
        s(a.name),
        s(a.no)
    ]);
    (doc as any).autoTable({ head, body, startY: 35, styles: { font: 'Cairo', halign: 'right' } });
    doc.save(`Trial_Balance_${date}.pdf`);
};

export const exportBalanceSheetToPdf = (data: any, settings: Settings, date: string) => {
    const doc = getArabicDoc('الميزانية العمومية', `بتاريخ: ${formatDate(date)}`, settings);
    const head = [['المبلغ', 'البند']];
    const body = [
        { content: 'الأصول', colSpan: 2, styles: { halign: 'center', fontStyle: 'bold', fillColor: [230, 230, 230] } },
        ...data.assets.map((a: any) => [formatCurrency(a.amount, settings.currency), a.name]),
        { content: 'الخصوم وحقوق الملكية', colSpan: 2, styles: { halign: 'center', fontStyle: 'bold', fillColor: [230, 230, 230] } },
        ...data.liabilities.map((l: any) => [formatCurrency(l.amount, settings.currency), l.name]),
        ...data.equity.map((e: any) => [formatCurrency(e.amount, settings.currency), e.name]),
    ];
    (doc as any).autoTable({ head, body, startY: 35, styles: { font: 'Cairo', halign: 'right' } });
...
export const exportAgedReceivablesToPdf = (data: any, settings: Settings, date: string) => {
    const doc = getArabicDoc('أعمار الذمم (Aging)', `بتاريخ: ${formatDate(date)}`, settings);
    const head = [['الإجمالي', '+90 يوم', '60-90 يوم', '30-60 يوم', '0-30 يوم', 'المستأجر']];
    const body = data.map((d: any) => [
        formatCurrency(d.totalDue, settings.currency),
        formatCurrency(d.ninetyPlus, settings.currency),
        formatCurrency(d.sixtyPlus, settings.currency),
        formatCurrency(d.thirtyPlus, settings.currency),
        formatCurrency(d.current, settings.currency),
        s(d.tenantName)
    ]);
    (doc as any).autoTable({ head, body, startY: 35, styles: { font: 'Cairo', halign: 'right' } });
    doc.save(`Aging_Report_${date}.pdf`);
};
