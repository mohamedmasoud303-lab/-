...
const AuditLog: React.FC = () => {
    ...
    const handleRestore = async (snapshot: Snapshot) => {
        if (window.confirm(`تحذير! هل أنت متأكد من أنك تريد استعادة النظام إلى الحالة التي كان عليها في "${formatDateTime(new Date(snapshot.ts).toISOString())}"؟ سيتم فقدان جميع التغييرات التي تمت بعد هذا التاريخ. لا يمكن التراجع عن هذا الإجراء.`)) {
            ...
            try {
                await restoreBackup(snapshotDataString);
            } catch (error) {
                // Error is already toasted inside restoreBackup, so we just log it here.
    ...
    const filteredLog = useMemo(() => {
        return db.auditLog.filter(log => {
            const matchesSearch = searchTerm === '' || (log.entityId && log.entityId.includes(searchTerm)) || log.note.toLowerCase().includes(searchTerm.toLowerCase());
                        ...
                        <tbody>
                            {filteredLog.slice(0, 200).map(log => (
                                <tr key={log.id} className="bg-card hover:bg-background">
                                    ...
                                    <td className="px-6 py-4 text-xs font-mono border border-border" title={log.entityId || ''}>
                                        {log.entityId && typeof log.entityId === 'string'
